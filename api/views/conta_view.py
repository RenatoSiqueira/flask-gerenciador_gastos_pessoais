...
class ContaDetail(Resource):
  def get(self, id):
    conta = conta_service.buscar_conta(id)
    if conta is None:
      return make_response(jsonify({"mensagem": "Conta não encontrada"}), 404)
    cs = conta_schema.ContaSchema()
  ...
  def put(self, id):
    conta_bd = conta_service.buscar_conta(id)
    if conta_bd is None:
      return make_response(jsonify({"mensagem": "Conta não encontrada"}), 404)
    cs = conta_schema.ContaSchema()
    validate = cs.validate(request.json)
    if validate:
      ...
      saldo = request.json["saldo"]
      conta_nova = conta.Conta(nome=nome, descricao=descricao, saldo=saldo)
      result = conta_service.atualizar_conta(conta_bd, conta_nova)
      return make_response(cs.jsonify(result), 201)
